...
public class HttpRoutersComposeFutureInvestigationVerticle extends AbstractVerticle {
  ...
  private List<Future> getJsonObjectFuture(RoutingContext routingContext) {
    return ((List<Future>) routingContext.get(FUTURE_KEY));
  }
//
...
//
  private void createRoutingContextHandler2(RoutingContext routingContext) {
    ...
    getJsonObjectFuture(routingContext).add(localJsonObjectFuture);
  ...
  private void configureRouter(Router router, Handler<AsyncResult<Router>> resultHandler) {
    ...
    router.route().handler(routingContext -> {
      routingContext.put(FUTURE_KEY, new ArrayList<Future<JsonObject>>());
      routingContext.next();
    });
    ...
    router.route().handler(routingContext -> {
      CompositeFuture.join(getJsonObjectFuture(routingContext)).setHandler(ar -> {
        HttpServerResponse response = routingContext.response();
        if (ar.succeeded()) {
          response
            .setStatusCode(200)
            .end(new JsonObject().put("success", true).put("results", ar.result().list()).encodePrettily());
        } else {
          response
            .setStatusCode(500)
